...
namespace SkyBlueSofa\C2CBlanketFramework\Repositories;
...
use SkyBlueSofa\C2CBlanketFramework\Facades\TemperatureBlanketDotCom;

class TemperatureBlanketRepository
{
    protected string $prefix = 'c2c.c2c.';
    ...
    {
        $designName = Config::get('c2c.c2c.design');
        ...
        elseif (empty($designScheme = Config::get('c2c.c2c.designs.'.$designName))) {
            if (! File::exists(config_path('c2c/designs/'.$designName.'.txt'))) {
                throw new FileNotFoundException("Design Scheme '".$designName.".txt' cannot be located in 'config/c2c/designs/' folder");
            }
        }

        // Finally, try to load a standard design from the base config
        else {
            $designScheme = Config::get('c2c.c2c.designs.'.$designName);
        }
    ...
    {
        $designName = Config::get('c2c.c2c.color');

        if ($designScheme = self::temperatureBlanketDotComDesignContents($designName)) {
            return $designScheme['colors'];
        } elseif (empty($designScheme = Config::get('c2c.c2c.colors.'.$designName))) {
            if (! File::exists(config_path('c2c/colors/'.$designName.'.txt'))) {
                throw new FileNotFoundException("Color Scheme '".$designName.".txt' cannot be located in 'config/colors/' folder");
            }
        } else {
            $designScheme = Config::get('c2c.c2c.colors.'.$designName);
        }

        if (empty($designScheme)) {
            throw new DomainException("Color Scheme '".$designName."' cannot be located in 'config/c2c/c2c.php' file");
        }
